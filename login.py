...
win.geometry("300x100+400+100")
win.resizable(0,0)

f1 = Frame()
f1.pack()
Label(f1,text="User Id: ").grid(row=0,column=0)
idVar = StringVar()
...
f2 = Frame()
f2.pack()
Button(f2,text="Submit",width="12",command = lambda:check(idVar.get().strip(),passVar.get().strip())).grid(row=2,column=0)
Button(f2,text="Back",width="12",command = goBack).grid(row=2,column=1)
readFile.close()
win.mainloop()
